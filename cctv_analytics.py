...
def load_local_env(path: str | Path | None = None) -> None:
    ...
    """
    # Resolve from the repository/script root, not the caller's current
    # directory, so systemd, Docker and shell launches load the same profile.
    env_path = (
...
class Settings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.frame_skip < 0 or self.loiter_seconds < 0:
            raise ValueError("frame_skip and loiter_seconds cannot be negative")
        if self.crowd_window_seconds <= 0 or self.alert_hold_seconds < 0:
            raise ValueError("crowd window must be positive and alert hold cannot be negative")
        if self.crowd_min_people < 1 or self.crowd_jump < 1:
            raise ValueError("crowd thresholds must be positive")
        if self.alert_confirm_frames < 1 or self.alert_cooldown_seconds < 0:
            raise ValueError("alert confirmation/cooldown values are invalid")
        if (
            self.evidence_seconds < 0
            or self.display_width < 1
            or self.display_height < 1
        ):
            raise ValueError("evidence duration and display dimensions are invalid")
...
class LatestFrameReader:
    ...
    def __init__(self, source: str) -> None:
        ...
        self._is_local_file = isinstance(self.source, str) and Path(self.source).is_file()
    ...
    def _open(self) -> cv2.VideoCapture:
        # Native webcams work most reliably with the operating system's
        # default capture backend; files and RTSP streams use FFmpeg.
        capture = (
    ...
    def _run(self) -> None:
        retry_delay = 2.0
        # Keep this across capture reopens so a looping file or camera reboot
        # that restarts PTS at zero still invalidates temporal analytics.
        ...
        while not self._stop.is_set():
            ...
            if not self._capture.isOpened():
                ...
                LOGGER.warning(
                    "Cannot open video source; retrying in %.0f seconds", retry_delay
                )
            ...
            playback_pts_origin: Optional[float] = None
            ...
            while not self._stop.is_set():
                ...
                if self._is_local_file and pts_seconds is not None:
                    # A file decoder can run far ahead of inference. Pace it
                    # from PTS so the threaded layer behaves like a live feed.
                    if playback_pts_origin is None:
                        playback_pts_origin = pts_seconds
                    due_at = playback_started + (pts_seconds - playback_pts_origin)
                    self._stop.wait(max(0.0, due_at - time.monotonic()))
                if (
                    pts_seconds is not None
                    and previous_pts is not None
                    and pts_seconds + 0.050 < previous_pts
                ):
                    self._producer_epoch += 1
                    LOGGER.info(
                        "Scene/timestamp discontinuity detected; temporal state will reset"
                    )
...
class AnomalyDetector:
    ...
    def evaluate(
        ...
        """Evaluate all tracked candidate classes using stream PTS seconds."""
        active_ids = {track_id for track_id, _, _ in candidates}
        loitering_ids: set[int] = set()

        for track_id, _class_id, foot_point in candidates:
        ...
        self.crowd_history.append((now, people_count))
        ...
        baseline = min(previous_counts) if previous_counts else people_count
        sudden_crowd = (
            people_count >= self.settings.crowd_min_people
            and people_count - baseline >= self.settings.crowd_jump
        )
...
class PrahariApp:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self.manual_alert_until = float("-inf")
    ...
    def _zone_for_frame(self, frame: np.ndarray) -> np.ndarray:
        if self.zone_editing and len(self.zone_draft) >= 3:
            ...
            return np.array(
                [(int(x * width), int(y * height)) for x, y in self.zone_draft],
                np.int32,
            )
    ...
    def _apply_remote_settings(self) -> None:
        ...
        if not values:
            return
        self.settings.confidence = max(
            0.1, min(float(values.get("confidence", self.settings.confidence)), 0.9)
        )
        self.settings.loiter_seconds = max(
            1.0,
            min(float(values.get("loiter_seconds", self.settings.loiter_seconds)), 600.0),
        )
        self.settings.crowd_min_people = max(
            2,
            min(int(values.get("crowd_min_people", self.settings.crowd_min_people)), 100),
        )
        self.settings.crowd_jump = max(
            1, min(int(values.get("crowd_jump", self.settings.crowd_jump)), 50)
        )
    ...
    def _analytics_time(self) -> float:
        ...
        if self.reader.stream_epoch != self._last_stream_epoch:
            ...
            self.trails.clear()
            self.previous_alerts.clear()
            self.manual_alert_until = float("-inf")
            self.evidence.handle_discontinuity()
        ...
        if self.reader.latest_pts_seconds is not None:
            return self.reader.latest_pts_seconds
    ...
    def _draw_result(self, frame: np.ndarray, result, zone: np.ndarray, now: float) -> np.ndarray:
        ...
        if boxes is not None and len(boxes):
            ...
            for box, class_id, score, track_id in zip(coordinates, classes, scores, ids):
                ...
                if class_id == PERSON_CLASS:
                    # Count a visible person immediately; a persistent ID is
                    # only mandatory for dwell-time/loitering state.
                    people_count += 1
                if track_id >= 0:
                    candidates.append((track_id, class_id, foot_point))

        alerts, loitering_ids = self.anomalies.evaluate(
            candidates, people_count, zone, now
        )
        if now < self.manual_alert_until:
            alerts.append("DEMO ALERT - OPERATOR TEST")
        if any("LOITERING" in item for item in alerts):
            severity = "critical"
        elif alerts:
            severity = "high"
        elif any(
            class_id in SUSPICIOUS_OBJECT_CLASSES
            for _, class_id, _, _ in detections
        ):
            severity = "medium"
        else:
            severity = "low"
        self.telemetry = {
            "people": people_count,
            "vehicles": sum(
                class_id in VEHICLE_CLASSES for _, class_id, _, _ in detections
            ),
            "objects": sum(
                class_id in SUSPICIOUS_OBJECT_CLASSES
                for _, class_id, _, _ in detections
        ...
        for track_id, class_id, score, (x1, y1, x2, y2) in detections:
            ...
            if class_id in SUSPICIOUS_OBJECT_CLASSES:
                ...
                if not is_alert:
                    color = (0, 165, 255)
            text = (
                f"{label} {score:.2f} ID:{track_id}"
                if track_id >= 0
                else f"{label} {score:.2f}"
            )
        ...
        status = (
            f"{self.settings.camera_id} {connection} | People:{people_count} "
            f"Vehicles:{self.telemetry['vehicles']} | FPS:{fps:.1f} "
            f"Latency:{self.inference_ms:.0f}ms Drops:{self.reader.dropped_frames} "
            f"Reconnects:{self.reader.reconnect_count}"
        )
        cv2.putText(
            frame,
            status,
    ...
    def _write(self, frame: np.ndarray) -> None:
        ...
        if self.writer is None:
            ...
            if not self.writer.isOpened():
                failed_path = self.recording_path
                self.writer.release()
                self.writer = None
                self.recording_enabled = False
                LOGGER.error("Could not create recording: %s", failed_path)
                return
    ...
    def _mouse_event(self, event: int, x: int, y: int, _flags: int, _param) -> None:
        ...
        point = (max(0.0, min(x / width, 1.0)), max(0.0, min(y / height, 1.0)))
        duplicate = bool(self.zone_draft) and sum(
            (a - b) ** 2 for a, b in zip(point, self.zone_draft[-1])
        ) < 0.0001
        if duplicate:
            LOGGER.info("Ignored duplicate zone point")
            return
        if len(self.zone_draft) >= 32:
            LOGGER.warning("Zone point limit reached")
            return
        self.zone_draft.append(point)
    ...
    def run(self) -> None:
        ...
        try:
            while self.running:
                ...
                # Observe PTS discontinuities before tracking/inference so a
                # hard scene cut cannot inherit the previous scene's IDs.
                analytics_now = self._analytics_time()
                ...
                annotated = self._draw_result(
                    working_frame, result, self._zone_for_frame(working_frame), analytics_now
                )
                current_alerts = set(self.telemetry["alerts"])
                for alert_name in current_alerts - self.previous_alerts:
                    try:
                        self.evidence.start_alert(
                            alert_name, annotated, self.telemetry, analytics_now
                        )
                    except OSError as exc:
                        # Disk/codec failures must be visible but must not take
                        # the live safety feed down.
                        LOGGER.error("Could not persist alert evidence: %s", exc)
                self.previous_alerts = current_alerts
                self.evidence.add_frame(annotated, analytics_now)
                ...
                if not self.settings.headless:
                    ...
                    if character == "s":
                        ...
                        self.manual_alert_until = analytics_now + self.settings.alert_hold_seconds
                    elif character == "r":
                        ...
                        self.zone_draft = []
                        LOGGER.info(
                            "Zone editor ON: click at least 3 polygon points, then press Enter"
                        )
                        ...
                        if len(self.zone_draft) >= 3:
                            try:
                                ...
                                LOGGER.warning("Zone not saved: %s", exc)
                            else:
                                self.anomalies = AnomalyDetector(self.settings)
                                self.zone_editing = False
                        else:
                            LOGGER.warning("Add at least three points before saving the zone")
                    elif character == "c" and self.zone_editing:
                        self.zone_editing = False
                        self.zone_draft = []
        finally:
            self.reader.stop()
...
def parse_args() -> Settings:
    ...
    parser.add_argument(
        "--frame-skip", type=int, default=0, help="Skip N frames between inferences"
    )
    ...
    parser.add_argument(
        "--snapshots-dir", default="output/snapshots", help="Folder used by the S key"
    )
    parser.add_argument(
        "--camera-id", default="CAM01", help="Camera ID included in evidence and telemetry"
    )
    parser.add_argument(
        "--zone-file", default="config/cam01_zone.json", help="Saved restricted-zone polygon"
    )
    parser.add_argument(
        "--evidence-dir", default="output/evidence", help="Automatic alert evidence folder"
    )
    parser.add_argument("--evidence-seconds", type=float, default=10.0, help="Alert clip duration")
    parser.add_argument(
        "--alert-confirm-frames",
        type=int,
        default=2,
        help="Consecutive rule hits required",
    )
    parser.add_argument(
        "--alert-cooldown-seconds",
        type=float,
        default=20.0,
        help="Repeat alert cooldown",
    )
    parser.add_argument(
        "--enhance-low-light",
        action="store_true",
        help="Start with night enhancement enabled",
    )
    parser.add_argument(
        "--dashboard-url",
        default=os.getenv("DASHBOARD_URL"),
        help="Optional command dashboard URL",
    )
    parser.add_argument(
        "--dashboard-token",
        default=os.getenv("DASHBOARD_TOKEN"),
        help="Optional telemetry bearer token",
    )
    parser.add_argument(
        "--roi-only",
        action="store_true",
        help="Count and alert only inside the restricted zone",
    )
    parser.add_argument(
        "--trail-length",
        type=int,
        default=20,
        help="Tracked path length in processed frames",
    )
